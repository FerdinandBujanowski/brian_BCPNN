...
model = None
if b_from_file:
    model = TwoSynTypeNetwork(N_H, N_M, N_pyr=N_pyr, N_BA=N_BA, namespace=fiebig_namespace, eqs=fiebig_equations, filepath=filepath)
else:
...
pattern_1_train = spils.get_pattern_population_train(spikemon, model, pattern_list.patterns[0], t_total, defaultclock.dt)
pattern_1_freqs = spils.get_firing_rate_estimate(pattern_1_train, N_M*N_pyr, 0.1, t_total/ms, kernel_size=15)
ax2.plot(curmon_2.t/second, pattern_1_freqs, label='Pattern 1 Avg Freqs', c='g')

pattern_2_train = spils.get_pattern_population_train(spikemon, model, pattern_list.patterns[1], t_total, defaultclock.dt)
pattern_2_freqs = spils.get_firing_rate_estimate(pattern_2_train, N_M*N_pyr, 0.1, t_total/ms, kernel_size=15)

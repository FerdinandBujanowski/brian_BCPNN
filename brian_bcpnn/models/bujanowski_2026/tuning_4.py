# SIMULATION PURPOSE: Get single intra-MC BA IPSP to around -2.5 mV (at V_m=-70mV)
...
j_list = [j for i,j in zip(model.S_BP.i, model.S_BP.j) if i == 0]
# add BA voltage monitor
volmon = model.add_statemon(variables=['V_m'], record=j_list)
# add BA voltage monitor
bas_volmon = StateMonitor(model.BA, variables=['V_m'], record=0)
...
for i in range(5):
    ...
    model.BA.V_m[0] = -50*mV

    # run again
    model.run(50*ms, verbose=False)

# plot voltages
fig, [ax1, ax2] = plt.subplots(2, 1, sharex=True, gridspec_kw={'height_ratios': (1, 5)})
trains.get_full_train(ax1, basmon, model.N, 310*ms)
ax2.plot(bas_volmon.t/ms, bas_volmon[0].V_m/mV, label='neuron 0', color='b')
ax2.plot(volmon.t/ms, volmon[j_list[1]].V_m/mV, label='connected PYR cell', color='k')

...
def ltp_ltd_func(spike_indices, spike_times): #spike_indices = list of spike indices
    ...
    for i in range(len(spike_indices), 80): #-1 
        if spike_indices[i] != spike_indices[i+1]: #if two following elem not the same, we are only interested in cases 0,1 or 1,0 
            ...
            pre_w = w_array[n_iterations-1][int(spike_times[pre_ind]/dt)] #the weight at specific spike time of pre divided by timestep dt 
            print('w(pre):', pre_w)
            post_w = w_array[n_iterations-1][int(spike_times[post_ind]/dt)] #the weight at specific spike time of post divided by timestep dt 
            print('w(post):', post_w)
            # I want to calculate the synaptic weight difference (in %)
            wchange = ((post_w - pre_w)/pre_w) * 100 
